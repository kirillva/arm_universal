...
export const NumberFilter = ({ column: { filterValue, setFilter }, className, allowNegative = true }) => {
	...
	const error = (!allowNegative && Number(value) < 0) ? 'Значение не может быть отрицательным' : '';
	...
	return (
		<TextField
			...
			onChange={e => {
				...
				if (!error) {
					applyFilterDebounced(filterValue, e.target.value, setFilter, Operators.number);
				}
			}}
			onKeyDown={e => {
				if (!error) { 
					if (e.key === 'Enter') {
						applyFilter(filterValue, e.target.value, setFilter, Operators.number);
...
export const StringFilter = ({ column: { filterValue, setFilter }, className }) => {
	const [value, setValue] = useState(filterValue ? filterValue.value : '');
	return (
		<TextField
			variant="outlined"
...
export const UserFilter = ({ column: { filterValue, setFilter }, className }) => {
	const [value, setValue] = useState(filterValue ? filterValue.value : '');
	return (
		<TextField
			disabled
			...
			onChange={e => {
				setValue(e.target.value);
				applyFilterDebounced(filterValue, e.target.value, setFilter, Operators.user);
			}}
			onKeyDown={e => {
				if (e.key === 'Enter') {
...
export const BoolFilter = ({ column: { filterValue, setFilter }, className }) => {
	...
	return (
		<TextField
			...
			// 	className: styles.baseField
			// }}
			onChange={e => setFilter({ value: e.target.value, operator: Operators.bool })}
		>
...
export const StatusFilter = ({ column: { filterValue, setFilter }, className }) => {
